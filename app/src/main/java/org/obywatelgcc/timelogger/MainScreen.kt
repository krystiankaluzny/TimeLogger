...
import org.obywatelgcc.timelogger.ui.theme.isWidthExpanded
...
fun MainScreen(windowSizeClass: WindowSizeClass) {
    ...
    if (windowSizeClass.isWidthExpanded()) {
        PermanentNavigationDrawer(
            drawerContent = {
                PermanentDrawerContent(navController, appName, drawerState, navItems, navBackStackEntry)
            },
        ) {
            ContentView(windowSizeClass, appName, drawerState, snackbarHostState, navController)
        }
    } else {
        ModalNavigationDrawer(
            drawerState = drawerState,
            drawerContent = {
                ModalDrawerContent(navController, appName, drawerState, navItems, navBackStackEntry)
            }
        ) {
            ContentView(windowSizeClass, appName, drawerState, snackbarHostState, navController)
        }
...
fun PermanentDrawerContent(
    ...
    PermanentDrawerSheet(
        ...
        drawerContainerColor = TimeLoggerTheme.colorScheme.surfaceContainerLow
    ) {
        DrawerContent(navController, appName, drawerState, navigationItems, navBackStackEntry)
    }
}
...
fun DrawerContent(
    ...
    navigationItems.forEach { item ->
        ...
        NavigationDrawerItem(
            ...
            label = { Text(text = item.title, fontSize = TimeLoggerTheme.values.textFieldFontSize) },
            selected = isSelected,
            onClick = {
                if (!isSelected) {
...
private fun ContentView(
    ...
    Scaffold(
        topBar = { TopBarView(windowSizeClass, appName, drawerState) },
...
@Composable
private fun TopBarView(windowSizeClass: WindowSizeClass, appName: String, drawerState: DrawerState) {
